...
void FUxtIconBrushCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> PropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
...
void FUxtIconBrushCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> PropertyHandle, IDetailChildrenBuilder& Builder, IPropertyTypeCustomizationUtils& CustomizationUtils)
{
			...
			Builder.AddCustomRow(Description)
				...
				[
					...
					.IsEnabled((MakeAttributeLambda([=] 
					{
						...
						const bool Success = PropertyHandle->GetChildHandle(ChildName)->GetValue(Enum) == FPropertyAccess::Success;
						return Success && (static_cast<EUxtIconBrushContentType>(Enum) == EUxtIconBrushContentType::UnicodeCharacter || 
										   static_cast<EUxtIconBrushContentType>(Enum) == EUxtIconBrushContentType::String);
					})))
					.OnClicked(this, &FUxtIconBrushCustomization::OnShowEditor, PropertyHandle)
					[
						SNew(STextBlock)
						.Font(IDetailLayoutBuilder::GetDetailFont())
...
{
	if (UWidgetBlueprint* Blueprint = Cast<UWidgetBlueprint>(StaticLoadObject(UWidgetBlueprint::StaticClass(), nullptr, TEXT("/UXTools/Utilities/IconBrushEditor/UW_IconBrushEditor"))))
	{
		if (UEditorUtilitySubsystem* EditorUtilitySubsystem = GEditor->GetEditorSubsystem<UEditorUtilitySubsystem>())
		{
			FName TabID;
			// Note the use of static_cast<> from UWidgetBlueprint to UEditorUtilityWidgetBlueprint because UEditorUtilityWidgetBlueprint is not exported
			// by the BLUTILITY_API and the StaticClass method required by Cast<> cannot be used.
			if (UUxtIconBrushEditorUtilityWidget* Widget = Cast<UUxtIconBrushEditorUtilityWidget>(EditorUtilitySubsystem->SpawnAndRegisterTabAndGetID(static_cast<UEditorUtilityWidgetBlueprint*>(Blueprint), TabID)))
			{
