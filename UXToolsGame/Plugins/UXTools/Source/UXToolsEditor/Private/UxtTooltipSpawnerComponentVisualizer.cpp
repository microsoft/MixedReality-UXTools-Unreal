	...
	const UActorComponent* Component, const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	...
	if (AActor* Owner = TooltipSpawner->GetOwner())
	{
		...
		const FVector LineStart = Owner->GetActorLocation();
		...
		const FVector Vertices[] = {
			FVector(-Depth, Width, Height), FVector(-Depth, Width, -Height), FVector(-Depth, -Width, -Height),
