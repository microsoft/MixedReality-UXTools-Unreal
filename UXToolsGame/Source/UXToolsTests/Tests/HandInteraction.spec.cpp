...
void HandInteractionSpec::Define()
{
	...
	LatentIt(
		"should activate near pointer if close to a target",
		[this](const FDoneDelegate& Done)
		{
			...
			FrameQueue.Enqueue(
				[this, Done]
				{
					...
					FVector Normal;
					TestEqual(
						TEXT("Near pointer focusing target"), NearPointer->GetFocusedGrabTarget(ClosestPoint, Normal), (UObject*)Target);
	...
	LatentIt(
		"should activate far pointer if not close to a target",
		[this](const FDoneDelegate& Done)
		{
	...
	LatentIt(
		"should deactivate pointer when hand is not in the pointing pose",
		[this](const FDoneDelegate& Done)
		{
			FrameQueue.Enqueue(
	...
	LatentIt(
		"should deactivate pointers when tracking is lost",
		[this](const FDoneDelegate& Done)
		{
			FrameQueue.Enqueue(
				[this]
	...
	LatentIt(
		"should reset proximity detection when tracking is lost",
		[this](const FDoneDelegate& Done)
		{
			UxtTestUtils::GetTestHandTracker().SetAllJointPositions(NearPoint);
	...
	LatentIt(
		"should not transition between near and far interaction modes when a pointer is locked",
		[this](const FDoneDelegate& Done)
		{
			UxtTestUtils::GetTestHandTracker().SetAllJointPositions(NearPoint);
	...
	LatentIt(
		"should not affect its far beam's transform",
		[this](const FDoneDelegate& Done)
		{
			FrameQueue.Enqueue(
				[this]
