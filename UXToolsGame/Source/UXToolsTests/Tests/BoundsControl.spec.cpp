...
void BoundsControlSpec::Define()
{
	BeforeEach([this] {
		...
		SetupEventCaptureComponent(
			Cast<UUxtGrabTargetComponent>(Target->GetBoundsControlActor()->GetComponentByClass(UUxtGrabTargetComponent::StaticClass())));
	...
	Describe("Far interaction", [this] {
		...
		LatentIt("should focus collision box", [this](const FDoneDelegate& Done) {
			...
			FrameQueue.Enqueue([this] {
				...
				const float OffsetZ = Bounds.GetCenter().Z + (Bounds.GetExtent().Z / 2); // Avoid both top and center affordances
...
{
	LatentIt("should scale affordance primitives based on distance to actor's center", [this](const FDoneDelegate& Done) {
		EnqueueNoFocusedAffordanceChecks();
		FrameQueue.Enqueue([this] {
		...
		FrameQueue.Enqueue([this] {
			...
			TestTrue(
				"Affordance was scaled after actor resize", TargetPrimitive->GetRelativeScale3D().Size() > InitialAffordanceScale.Size());
		});
		FrameQueue.Enqueue([Done] { Done.Execute(); });
	});
}
...
{
	Describe("without external constraints", [this] {
		LatentIt("clamps to min scale", [this](const FDoneDelegate& Done) {
			EnqueueNoFocusedAffordanceChecks();
			FrameQueue.Enqueue([this] {
			...
			FrameQueue.Enqueue([this] {
				TestEqual(
					"Scale was clamped to min value", Actor->GetActorScale3D(), InitialActorTransform.GetScale3D() * Target->GetMinScale());
			});
			FrameQueue.Enqueue([Done] { Done.Execute(); });
		});

		LatentIt("clamps to max scale", [this](const FDoneDelegate& Done) {
			EnqueueNoFocusedAffordanceChecks();
			FrameQueue.Enqueue([this] {
				Target->SetRelativeToInitialScale(true);
				TestEqual("Affordance didn't receive focus beforehand", EventCaptureComponent->GetFocusEnterCount(InteractionMode), 0);
			...
			FrameQueue.Enqueue([this] {
				TestEqual(
					"Scale was clamped to max value", Actor->GetActorScale3D(), InitialActorTransform.GetScale3D() * Target->GetMaxScale());
			});
			FrameQueue.Enqueue([Done] { Done.Execute(); });
		});

		LatentIt("prevents scaling with min/max scale = 1", [this](const FDoneDelegate& Done) {
			EnqueueNoFocusedAffordanceChecks();
			FrameQueue.Enqueue([this] {
				Target->SetRelativeToInitialScale(true);
			...
			FrameQueue.Enqueue([this] { TestEqual("Doesn't scale down", Actor->GetActorScale3D(), InitialActorTransform.GetScale3D()); });
			FrameQueue.Enqueue([this] { LeftHand.Translate(FVector(0, 2000, 0)); });
			FrameQueue.Enqueue([this] { TestEqual("Doesn't scale up", Actor->GetActorScale3D(), InitialActorTransform.GetScale3D()); });
			FrameQueue.Enqueue([Done] { Done.Execute(); });
		});

		LatentIt("doesn't rotate with corner affordance", [this](const FDoneDelegate& Done) {
			EnqueueNoFocusedAffordanceChecks();
			FrameQueue.Enqueue([this] {
				TestEqual("Affordance didn't receive focus beforehand", EventCaptureComponent->GetFocusEnterCount(InteractionMode), 0);
				LeftHand.SetTranslation(TargetPrimitive->GetComponentLocation(), /* bApplyOffset = */ true);
		...
		LatentIt("rotates around UpVector", [this](const FDoneDelegate& Done) {
			FrameQueue.Enqueue([this] { TargetPrimitive = Target->GetAffordancePrimitive(EUxtAffordancePlacement::EdgeFrontRight); });
			EnqueueRotationAroundAxisChecks(FVector::UpVector);
			FrameQueue.Enqueue([Done] { Done.Execute(); });
		});

		LatentIt("rotates around RightVector", [this](const FDoneDelegate& Done) {
		...
		LatentIt("rotates around ForwardVector", [this](const FDoneDelegate& Done) {
			FrameQueue.Enqueue([this] {
	...
	Describe("with rotation constraint", [this] {
		...
		Describe("Y allowed in world space", [this] {
			BeforeEach([this] {
	...
	Describe("with translation constraint", [this] {
		...
		Describe("restricting local Y|Z", [this] {
			...
			LatentIt("moves along local X", [this](const FDoneDelegate& Done) {
				FrameQueue.Enqueue([this] { TargetPrimitive = Target->GetAffordancePrimitive(EUxtAffordancePlacement::FaceFront); });
				EnqueueTranslationAlongAxisChecks(FVector::ForwardVector);
				FrameQueue.Enqueue([Done] { Done.Execute(); });
			});

			LatentIt("doesn't move along local Y", [this](const FDoneDelegate& Done) {
				FrameQueue.Enqueue([this] {
					// As this requires to target the affordance on the top face, rotate the actor and re-cache the initial transform
					Actor->SetActorRotation(FRotator(0, 90, 0));
					InitialActorTransform = Actor->GetActorTransform();
			...
			LatentIt("doesn't move along local Z", [this](const FDoneDelegate& Done) {
				FrameQueue.Enqueue([this] {
					// As this requires to target the affordance on the top face, rotate the actor and re-cache the initial transform
		...
		Describe("restricting world Y|Z", [this] {
			BeforeEach([this] {
				TranslationConstraint->ConstraintOnMovement = static_cast<uint32>(EUxtAxisFlags::Y | EUxtAxisFlags::Z);
			...
			LatentIt("moves along local Z", [this](const FDoneDelegate& Done) {
				FrameQueue.Enqueue([this] {
					// As this requires to target the affordance on the top face, rotate the actor and re-cache the initial transform
					Actor->SetActorRotation(FRotator(90, 0, 0));
					InitialActorTransform = Actor->GetActorTransform();
			...
			LatentIt("moves along local Y", [this](const FDoneDelegate& Done) {
				FrameQueue.Enqueue([this] {
					// As this requires to target the affordance on the top face, rotate the actor and re-cache the initial transform
					Actor->SetActorRotation(FRotator(0, 90, 0));
					InitialActorTransform = Actor->GetActorTransform();
...
{
	LatentIt("should raise enter/exit affordance focus", [this](const FDoneDelegate& Done) {
		EnqueueNoFocusedAffordanceChecks();
...
void BoundsControlSpec::EnqueueRotationAroundAxisChecks(const FVector& AllowedAxes)
{
	...
	FrameQueue.Enqueue([this, AllowedAxes] {
