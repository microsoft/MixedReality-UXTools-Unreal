...
void FFarPointerSpec::Define()
{
	...
	It("should have correct focus target",
	   [this]()
	   {
	...
	LatentIt(
		"should raise the correct events on far targets",
		[this](const FDoneDelegate& Done)
		{
	...
	LatentIt(
		"should become disabled/enabled on tracking loss/recovery",
		[this](const FDoneDelegate& Done)
		{
	...
	It("should report correct hit info",
	   [this]()
	   {
	...
	LatentIt(
		"should lock to current target when requested",
		[this](const FDoneDelegate& Done)
		{
			...
			FrameQueue.Enqueue(
				[this, NewTargetLocation, TargetActor]()
				{
	...
	LatentIt(
		"should release lock on tracking loss",
		[this](const FDoneDelegate& Done)
		{
			Pointer->SetFocusLocked(true);
			HandTracker->SetTracked(false);
