...
BEGIN_DEFINE_SPEC(GrabTargetComponentInteractionModeSpec, "UXTools.GrabTargetComponent", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::ApplicationContextMask)
			...
			LatentIt("should not be grabbed by near interaction when near mode is disabled", [this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue([this]
						{
							NearPointer = UxtTestUtils::CreateNearPointer(UxtTestUtils::GetTestWorld(), TEXT("GrabTestPointer"), Center + FVector(-15, 0, 0));
			...
			LatentIt("should not be grabbed by far interaction when far mode is disabled", [this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue([this, Done]
						{
							FarPointer = UxtTestUtils::CreateFarPointer(UxtTestUtils::GetTestWorld(), TEXT("FarTestPointer"),  FVector::ZeroVector, EControllerHand::Right);
