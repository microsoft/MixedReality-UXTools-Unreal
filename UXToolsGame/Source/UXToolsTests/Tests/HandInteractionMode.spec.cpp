...
void HandInteractionModeSpec::Define()
{
	Describe(
		...
		[this]
		{
			...
			LatentIt(
				"should not grab when near mode is disabled",
			...
			LatentIt(
				"should release grab when near mode is disabled",
				[this](const FDoneDelegate& Done)
				{
					EnqueueSetHandTrackerLocation(NearLocation);
					EnqueueGrabStatusChange(true);
			...
			LatentIt(
				"should not select when far mode is disabled",
				[this](const FDoneDelegate& Done)
				{
			...
			LatentIt(
				"should release selection when far mode is disabled",
				[this](const FDoneDelegate& Done)
				{
					EnqueueSetHandTrackerLocation(FarLocation);
					EnqueueSelectStatusChange(true);
