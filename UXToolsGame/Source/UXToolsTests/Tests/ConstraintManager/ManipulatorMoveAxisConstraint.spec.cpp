...
BEGIN_DEFINE_SPEC(ManipulatorMoveAxisConstraintSpec, "UXTools.GenericManipulator.Constraints", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::ApplicationContextMask)
			...
			BeforeEach([this]
				{
					...
					UUxtGenericManipulatorComponent* Manipulator = UxtTestUtils::CreateTestBoxWithComponent<UUxtGenericManipulatorComponent>(World, Center);
			...
			LatentIt("should restrict movement in X direction for far interaction", [this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue([this, Done]
						{
							FarPointerRight = UxtTestUtils::CreateFarPointer(UxtTestUtils::GetTestWorld(), TEXT("FarTestPointer"),  FVector::ZeroVector, EControllerHand::Right);
			...
			LatentIt("should restrict movement for two hand far interaction", [this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue([this, Done]
						{
							FarPointerRight = UxtTestUtils::CreateFarPointer(UxtTestUtils::GetTestWorld(), TEXT("FarTestPointer"), FVector::ZeroVector, EControllerHand::Right);
							FarPointerLeft = UxtTestUtils::CreateFarPointer(UxtTestUtils::GetTestWorld(), TEXT("FarTestPointerLeft"), LeftHandOffset, EControllerHand::Left);
							TargetActor->SetActorLocation(FVector(200, 0, 0));
						});
