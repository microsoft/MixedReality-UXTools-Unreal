...
BEGIN_DEFINE_SPEC(HandConstraintComponentSpec, "UXTools.HandConstraintComponent", EAutomationTestFlags::ProductFilter | EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext)
...
void HandConstraintComponentSpec::TestActorDistance(const FVector& ExpectedLocation, const FQuat& ExpectedRotation, float MaxActorDistance, float MaxActorAngle)
{
	...
	TestTrue("Actor linear distance below threshold", (HandConstraint->GetOwner()->GetActorLocation() - ExpectedLocation).Size() <= MaxActorDistance);
	TestTrue("Actor angular distance below threshold", HandConstraint->GetOwner()->GetActorQuat().AngularDistance(ExpectedRotation) <= MaxActorAngle);
}

void HandConstraintComponentSpec::EnqueueGoalLocationTest(EControllerHand Hand, EUxtHandConstraintZone Zone, const FVector& ExpectedGoalLocation)
{
...
void HandConstraintComponentSpec::EnqueueGoalRotationTest(EControllerHand Hand, EUxtHandConstraintZone Zone, const FQuat& ExpectedGoalRotation)
{
	FrameQueue.Enqueue([this, Hand, Zone]()
		{
			HandConstraint->Hand = Hand;
			...
			LatentIt("goal should have valid location in Hand-Rotation mode", [this](const FDoneDelegate& Done)
				{
					...
					EnqueueGoalLocationTest(EControllerHand::Right, EUxtHandConstraintZone::AboveFingerTips, FVector(76.2985f, 18.4254f, -14.6851f));
			...
			LatentIt("goal should have valid rotation in Hand-Rotation mode", [this](const FDoneDelegate& Done)
				{
					...
					EnqueueGoalRotationTest(EControllerHand::Left, EUxtHandConstraintZone::AboveFingerTips, FQuat(-0.1195f, 0.6793f, -0.2946f, 0.6614f));
					...
					EnqueueGoalRotationTest(EControllerHand::Right, EUxtHandConstraintZone::RadialSide, FQuat(-0.1195f, 0.6793f, -0.2946f, 0.6614f));
					EnqueueGoalRotationTest(EControllerHand::Right, EUxtHandConstraintZone::AboveFingerTips, FQuat(-0.1195f, 0.6793f, -0.2946f, 0.6614f));
					EnqueueGoalRotationTest(EControllerHand::Right, EUxtHandConstraintZone::BelowWrist, FQuat(-0.1195f, 0.6793f, -0.2946f, 0.6614f));
			...
			LatentIt("goal should have valid rotation in Look-At-Camera mode", [this](const FDoneDelegate& Done)
				{
					...
					EnqueueGoalRotationTest(EControllerHand::Left, EUxtHandConstraintZone::AboveFingerTips, FQuat(0.0485f, 0.0068f, 0.9891f, -0.1385f));
					...
					EnqueueGoalRotationTest(EControllerHand::Right, EUxtHandConstraintZone::AboveFingerTips, FQuat(0.0485f, 0.0068f, 0.9891f, -0.1385f));
					...
					FrameQueue.Enqueue([this]()
						{
							...
							TestActorDistance(FVector(26.0919f, -36.9541f, 90.0000f), FQuat(-0.4660f, 0.2414f, 0.7558f, 0.3916f), KINDA_SMALL_NUMBER, KINDA_SMALL_NUMBER);
						});
