...
{
	LatentIt("Should restrict rotation in Y axis with one hand", [this](const FDoneDelegate& Done)
		{
			...
			Constraint->RegisterComponent();
			FRotator ExpectedRotation = Target->TransformTarget->GetComponentTransform().Rotator();
	...
	LatentIt("Should restrict rotation in Y axis with two hands", [this](const FDoneDelegate& Done)
		{
			UUxtRotationAxisConstraint* Constraint = NewObject<UUxtRotationAxisConstraint>(Target->GetOwner());
			Constraint->ConstraintOnRotation = static_cast<int32>(EUxtAxisFlags::Y);
			Constraint->RegisterComponent();
			...
			FrameQueue.Enqueue([this]
				{
					const FQuat ExpectedRotation(FVector::ForwardVector, FMath::DegreesToRadians(90));
					const FTransform Result = Target->GetOwner()->GetTransform();
					TestEqual("Objects rotation didn't change with hands rotation", ExpectedRotation.Rotator(), Result.Rotator());
				});
