...
void GrabTargetComponentSpec::EnqueueInteractionTests()
{
	...
	Describe(
		...
		{
			LatentIt(
				"should not be grabbed when interaction mode is disabled",
				[this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue([this] { RightHand.SetGrabbing(true); });
					...
					FrameQueue.Enqueue(
						[this]
						{
							Target->InteractionMode = static_cast<int32>(
								InteractionMode == EUxtInteractionMode::Near ? EUxtInteractionMode::Far : EUxtInteractionMode::Near);
			...
			LatentIt(
				"should release when interaction mode is disabled during interaction",
				[this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue([this] { RightHand.SetGrabbing(true); });
	...
	Describe(
		...
		{
			LatentIt(
				"should be grabbed by one or two hands",
				[this](const FDoneDelegate& Done)
				{
			...
			LatentIt(
				"should only be grabbed by one hand",
				[this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue(
						[this]
			...
			LatentIt(
				"should only be grabbed by two hands",
				[this](const FDoneDelegate& Done)
				{
					FrameQueue.Enqueue(
						[this]
