...
void TooltipSpec::Define()
{
	Describe("Tooltip", [this]
	{
		...
		LatentIt("Creating/destroying the tooltip", [this](const FDoneDelegate& Done)
		{
			...
			{
				TestTrue("Make sure tooltip is constructed", TooltipActor->IsValidLowLevel());
			});
			FrameQueue.Skip();
		...
		LatentIt("Changing the tooltip widget", [this](const FDoneDelegate& Done)
		{
			...
			UBlueprint* WidgetBP = Cast<UBlueprint>(StaticLoadObject(UBlueprint::StaticClass(), nullptr, *WidgetBPAddress, nullptr, LOAD_None, nullptr));
			TestTrue("Loading Test widget", WidgetBP != nullptr);
			FrameQueue.Skip();
			FrameQueue.Enqueue([this, WidgetBP]
			{
				TooltipActor->WidgetClass = WidgetBP->GeneratedClass;
			});
			FrameQueue.Skip(2);
			FrameQueue.Enqueue([this, WidgetBP]
			{
		...
		LatentIt("Attaching tooltip to target", [this](const FDoneDelegate& Done)
		{
			...
			FrameQueue.Enqueue([this]
			{
				...
				TestTrue(TEXT("A tooltip attached to an actor should have a spline with a different start and end point."), Delta.Size() > 1.0);
			});
			FrameQueue.Enqueue([Done]
			{
				Done.Execute();
		...
		{				
			...
			{
				TooltipActor->SetTarget(SomeTargetActor, SomeTargetActor->GetRootComponent());
			});
			FrameQueue.Skip(2);
			FrameQueue.Enqueue([this, Offset]
			{
		...
		//		if (UAutomationBlueprintFunctionLibrary::TakeAutomationScreenshotInternal(World, "TestVisibilityScreenshot.jpg", "You should see the tooltip", Options))
		//		{
		//			FrameQueue.Pause();
		//		}
		//	});
		...
		//		});
		//		FAutomationScreenshotOptions Options(EComparisonTolerance::Low);
		//		if (UAutomationBlueprintFunctionLibrary::TakeAutomationScreenshotInternal(World, "TestInVisibilityScreenshot.jpg", "You shouldn't see the tooltip", Options))
		//		{
		//			FrameQueue.Pause();
		//		}
		//	});
		...
		//		});
		//		FAutomationScreenshotOptions Options(EComparisonTolerance::Low);
		//		if (UAutomationBlueprintFunctionLibrary::TakeAutomationScreenshotInternal(World, "TestCustomTextScreenshot.jpg", "The default text should have changed.", Options))
		//		{
		//			FrameQueue.Pause();
		//		}
		//	});
