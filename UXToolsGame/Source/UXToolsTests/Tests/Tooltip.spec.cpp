...
void TooltipSpec::Define()
{
	Describe(
		...
		[this]
		{
			...
			LatentIt(
				"Creating/destroying the tooltip",
			...
			LatentIt(
				"Changing the tooltip widget",
				[this](const FDoneDelegate& Done)
				{
					const FString WidgetBPAddress("/Game/UXToolsGame/Tests/Tooltip/W_TestText.W_TestText");
					UBlueprint* WidgetBP = Cast<UBlueprint>(
						StaticLoadObject(UBlueprint::StaticClass(), nullptr, *WidgetBPAddress, nullptr, LOAD_None, nullptr));
				...
				[this](const FDoneDelegate& Done)
				{
					...
					FrameQueue.Enqueue(
						[this]
						{
							...
							TestTrue(
								TEXT("A tooltip attached to an actor should have a spline with a different start and end point."),
								Delta.Size() > 1.0);
						});
					FrameQueue.Enqueue([Done] { Done.Execute(); });
				});
			...
			LatentIt(
				"Attaching to movable and making sure it follows",
				[this](const FDoneDelegate& Done)
				{
				...
				[this](const FDoneDelegate& Done)
				{
					...
					FrameQueue.Enqueue(
						[this, Offset]
						{
							...
							TestTrue(
								TEXT("The start position should not be 0 0 0 if the auto anchor is working."),
								SplineStartPos.Size() > 1.0f);
						});
					FrameQueue.Enqueue([Done] { Done.Execute(); });
				});
			...
			//		if (UAutomationBlueprintFunctionLibrary::TakeAutomationScreenshotInternal(World, "TestVisibilityScreenshot.jpg", "You
			// should
