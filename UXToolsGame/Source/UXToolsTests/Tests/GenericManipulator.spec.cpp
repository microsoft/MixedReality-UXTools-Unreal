...
void GenericManipulatorSpec::Define()
{
	...
	Describe(
		...
		[this]
		{
			It("should default to parent actor",
			   [this]
			   {
			...
			It("should target user defined component (same actor)",
			   [this]
			   {
			...
			It("should target user defined component (different actor)",
			   [this]
			   {
				   FComponentReference TargetComponentReference;
	...
	Describe(
		...
		[this]
		{
			...
			LatentIt(
				"should raise transform update events",
				[this](const FDoneDelegate& Done)
				{
					UGenericManipulatorTestComponent* EventCaptureComponent =
						NewObject<UGenericManipulatorTestComponent>(Target->GetOwner());
...
void GenericManipulatorSpec::EnqueueInteractionTests()
{
	...
	LatentIt(
		"should rotate around center with one hand",
		[this](const FDoneDelegate& Done)
		{
	...
	LatentIt(
		"should rotate around grab point with one hand",
		[this](const FDoneDelegate& Done)
		{
			const FQuat ExpectedRotation(FVector::ForwardVector, FMath::DegreesToRadians(90));
	...
	LatentIt(
		"should disable physics on grab",
		[this](const FDoneDelegate& Done)
		{
