		...
		[this]
		{
			BeforeEach(
				[this]()
				{
					...
					PalmUpConstraint->OnConstraintActivated.AddDynamic(
						HandConstraintListener, &UHandConstraintListener::OnConstraintActivated);
					PalmUpConstraint->OnConstraintDeactivated.AddDynamic(
			...
			LatentIt(
				"should only be active when facing camera",
			...
			LatentIt(
				"should only be active when facing the camera and hand is flat",
				[this](const FDoneDelegate& Done)
				{
					UxtTestUtils::GetTestHandTracker().SetTracked(true);
					UxtTestUtils::GetTestHandTracker().SetAllJointRadii(1.5f);
					...
					UxtTestUtils::GetTestHandTracker().SetJointPosition(
						IndexTipPositionFlat, EControllerHand::AnyHand, EHandKeypoint::IndexTip);
					UxtTestUtils::GetTestHandTracker().SetJointPosition(
						RingTipPositionFlat, EControllerHand::AnyHand, EHandKeypoint::RingTip);
			...
			LatentIt(
				"should never be active when hand is not flat",
				[this](const FDoneDelegate& Done)
				{
					UxtTestUtils::GetTestHandTracker().SetTracked(true);
					UxtTestUtils::GetTestHandTracker().SetAllJointRadii(1.5f);
					...
					UxtTestUtils::GetTestHandTracker().SetJointPosition(
						IndexTipPositionUnflat, EControllerHand::AnyHand, EHandKeypoint::IndexTip);
					UxtTestUtils::GetTestHandTracker().SetJointPosition(
						RingTipPositionUnflat, EControllerHand::AnyHand, EHandKeypoint::RingTip);
			...
			LatentIt(
				"should require gaze to activate",
				[this](const FDoneDelegate& Done)
				{
					UxtTestUtils::GetTestHandTracker().SetTracked(true);
					UxtTestUtils::GetTestHandTracker().SetAllJointRadii(1.5f);
					...
					PalmUpConstraint->Zone =
						EUxtHandConstraintZone::AboveFingerTips; // To avoid conflicts with the joints used for the hand plane.
					PalmUpConstraint->Hand = EControllerHand::Left;
					PalmUpConstraint->OffsetMode = EUxtHandConstraintOffsetMode::HandRotation;
					PalmUpConstraint->MaxPalmAngle = 80.0f;
